...
|------------|------|-------------|
| `CONCEPT:AX-OS.governance.audio` | Audio Processing Operations | MCP tool domain `audio_processing` — Action-routed dynamic tool registration |
| `CONCEPT:AX-OS.governance.audio-2` | Misc Operations | MCP tool domain `misc` — Action-routed dynamic tool registration |
...
This project integrates with `agent-utilities` via `CONCEPT:AU-ECO.messaging.native-backend-abstraction` (Unified Toolkit Ingestion). The `audio_transcriber` MCP server registers its tools with the agent-utilities FastMCP middleware, enabling automatic discovery, telemetry, and Knowledge Graph ingestion of all AUDIO-* concepts.
