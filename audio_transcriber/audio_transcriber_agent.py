...
AGENT_DESCRIPTION = "An intelligent agent for audio transcription and processing."
AGENT_SYSTEM_PROMPT = (
    "You are an Audio Transcription Expert Agent.\n"
    "You have access to tools for transcribing audio files.\n"
    "Your responsibilities:\n"
    "1. Analyze the user's request for audio transcription.\n"
    "2. Use the available tool `transcribe_audio` and relevant parameters to process the audio.\n"
    "3. Provide the transcription or processing results clearly.\n"
...
def create_agent_server(
    ...
    if skills_directory and os.path.exists(skills_directory):
        ...
        skills = [
            Skill(
                id="audio_transcriber_agent",
                name="Audio Transcriber Agent",
                description="General access to Audio Transcriber tools",
...
def usage():
    print(
        ...
        '  [Complex] audio-transcriber-agent --host "value" --port "value" --debug "value" --reload --provider "value" --model-id "value" --base-url "value" --api-key "value" --mcp-url "value" --mcp-config "value" --skills-directory "value" --web\n'
    )
