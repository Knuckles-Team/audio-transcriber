        ...
        try:
            ...
            if not os.path.isabs(config_path) and "/" not in config_path:
                ...
                try:
                    ...
                    pkg_res = files("audio_transcriber") / config_path
                    if pkg_res.is_file():
            ...
            if os.path.exists(config_path):
                ...
                logger.info(
                    f"audio-transcriber: Loaded {len(mcp_toolsets)} MCP servers from {config_path}"
                )
        except Exception as e:
            logger.error(
                f"audio-transcriber: Failed to load MCP config {effective_mcp_config}: {e}"
            )
